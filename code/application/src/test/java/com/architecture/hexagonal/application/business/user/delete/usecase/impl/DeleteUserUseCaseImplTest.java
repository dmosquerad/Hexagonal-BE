...
import com.architecture.hexagonal.application.testutils.user.delete.input.DeleteUserCommandTestDataBuilder;
...
class DeleteUserUseCaseImplTest {
  ...
  void execute_shouldDeleteUser_whenUserExists() throws ResourceNotFoundException {
    ...
    final DeleteUserCommand deleteUserCommand =
        DeleteUserCommandTestDataBuilder.builder().build().deleteUserCommand();

    Mockito.when(userRepositoryWritePort.deleteUser(deleteUserCommand.getUserId()))
        .thenReturn(Optional.of(user));

    final User result = deleteUserUseCaseImpl.execute(deleteUserCommand);
  ...
  void execute_shouldThrowResourceNotFoundException_whenUserNotFound() {
    ...
    AssertionsForClassTypes.assertThatThrownBy(
            () -> deleteUserUseCaseImpl.execute(deleteUserCommand))
