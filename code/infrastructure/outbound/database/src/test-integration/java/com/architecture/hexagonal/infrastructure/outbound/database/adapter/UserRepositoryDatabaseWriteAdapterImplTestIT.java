...
@SpringBootTest(classes = {UserRepositoryDatabaseWriteAdapterImpl.class})
@Transactional
@Sql(scripts = "/user/user.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_CLASS)
@ContextConfiguration(classes = TestApplication.class)
class UserRepositoryDatabaseWriteAdapterImplTestIT extends DatabaseIT {

  @Autowired
  UserRepositoryDatabaseWriteAdapterImpl userRepositoryDatabaseWriteAdapterImpl;
  ...
  void saveUser_shouldPersistUser_whenUserIsValid() {
    ...
    User result = userRepositoryDatabaseWriteAdapterImpl.saveUser(user);
  ...
  void deleteUser_shouldReturnUser_whenUserExists() {
    ...
    final Optional<User> result = userRepositoryDatabaseWriteAdapterImpl.deleteUser(user.getUser().getUserId());
